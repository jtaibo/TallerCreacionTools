...
def build_top_dirs_paths(project_root_folder):
    ...
    for t in topDirs:
        base_path = os.path.join(
            project_root_folder, topDirs[t]).replace('\\', '/')
        top_dirs_paths.append(base_path)
    return top_dirs_paths


def build_maya_workspace_path(project_root_folder):
    maya_workspace = os.path.join(
        project_root_folder, topDirs['pre&prod']).replace('\\', '/')
...
def build_maya_workspace(project_root_folder):
    ...
    maya_directories = []
    for m in mayaDict:
        subdir = os.path.join(maya_workspace, mayaDict[m]).replace('\\', '/')
...
def build_asset_subfolder_paths(project_root_folder, subfolder_name):
    ...
    asset_workspace_path = os.path.join(
        maya_workspace_path, subfolder_name).replace('\\', '/')
...
def build_libraries(project_root_folder):
    ...
    for lib_type in assetTypeDirsLibrary:
        asset_library_type_path = os.path.join(
            asset_library_path, lib_type).replace('\\', '/')
        sourceimages_library_type_path = os.path.join(
            sourceimages_library_path, lib_type).replace('\\', '/')
...
def create_new_asset(project_root_folder, asset_name, asset_type):
    ...
    for folder in maya_workspace:
        ...
        if maya_folder == 'assets':
            ...
            create_sourceimages_path(
                project_name, folder, asset_name, asset_type)
...
def create_asset_path(project_name, parent_path, asset_name, asset_type):
    ...
    for folder in dptDirs:
        asset_subfolder_path = os.path.join(
            asset_path, folder).replace('\\', '/')
    ...
    for shot in shot_list:
        ...
        try:
            if not os.path.exists(shot_path):
                ...
                print(
                    f"Shot {os.path.basename(shot_path)} already exists, process will continue skipping that folder")
                raise FileExistsError()
        finally:
            continue
...
def create_shot_subfolders(shot_num, shot_path):
    for subdir in prodDirs:
        ...
        shot_subfolder_path = os.path.join(
            shot_path, working_subdir).replace('\\', '/')
...
def create_sequence_path(project_root_folder, sequence_name):
    ...
    sequence_path = os.path.join(
        maya_workspace, f'scenes/{sequence_name}').replace('\\', '/')
...
def create_new_project_structure(project_path, project_ID="XXX", debug=False):
    ...
    sourceimages_subfolders = build_asset_subfolders(
        project_root_folder, 'sourceimages')

    library_subfolders = build_libraries(project_root_folder)

    project_folders = make_directories([project_root_folder], top_dirs, maya_workspace_folders,
                                       asset_subfolders, sourceimages_subfolders, library_subfolders)
