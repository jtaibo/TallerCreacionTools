...
class MasterOfCheckersUI(qtutils.CheckerWindow): 
    ...
    def __init__(self, parent=qtutils.getMayaMainWindow()): 
        ...
        ui_file = os.path.basename(__file__).split(".")[0].replace("_ui_old", "_old.ui")
        title = "Checker"
...
def run():
    ...
    global masterofcheckers_ui_old# define as a global variable, so there is only one window for this checker
    try:
        masterofcheckers_ui_old.close() # pylint: disable=E0601
        masterofcheckers_ui_old.deleteLater()
    except:
        pass
    masterofcheckers_ui_old = MasterOfCheckersUI()
    masterofcheckers_ui_old.populateUI()
    masterofcheckers_ui_old.checkAll()
    masterofcheckers_ui_old.show()
